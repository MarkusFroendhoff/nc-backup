...
<div id="ncbackup-admin" class="section">
	...
	<p class="settings-hint">
		<?php p($l->t('Connection to the nc-backup service. Start backups from the NC Backup app icon.')); ?>
	</p>
	...
	<p>
		<label for="ncbackup-token"><?php p($l->t('API token')); ?></label>
		<input type="text" id="ncbackup-token" autocomplete="off" spellcheck="false"
			placeholder="<?php p($_['has_token'] ? $l->t('Token is set – paste a new one to replace it') : $l->t('Paste the token from the nc-backup web UI')); ?>" />
	</p>
	<p>
